<template>
    <div :id="`app-${app.component}`" class="card" style="position: absolute" @click="bringForward()"
            ...
            <div class="d-flex">
                <div class="d-flex flex-row-reverse">
                    <div v-for="(button, i) in headerButtons" :key="i">
                        <button class="btn btn-sm btn-muted h-100" v-if="button.callback" @click="button.callback">
        ...
        <div class="card-body overflow-auto" :class="[
            padding ? 'pt-1' : 'p-0',
            overflowAuto ? 'overflow-auto' : 'overflow-visible',
        ]">
            <component :is="app.component" :sync="app.sync" :username="username" :awareness="awareness"
                @setHeaderButtons="headerButtons = clone($event)" @setPadding="padding = $event"
