<template>
  <div>
    <el-button type="primary" style="position: fixed; right: 3%; top: 3%; z-index:1000" @click="drawer = true">
      搜索消息
    </el-button> 
  ...
  <div>
		<vue-advanced-chat
			...
			:load-first-room="loadFirst"
      :room-id="roomId"
			:messages="JSON.stringify(messages)"
			:messages-loaded="messagesLoaded"
      :single-room="single"
      :user-tags-enabled="tag"
      :show-add-room="addRoom"
...
export default {
	...
	methods: {
		async init(){
			try {
            const { data: res } = await axios.get(`http://101.43.159.45:8001/api/chat/initial/2`);
                // if (res.code !== 200)
